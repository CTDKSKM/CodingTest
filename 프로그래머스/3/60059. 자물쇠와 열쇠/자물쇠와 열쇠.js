...
function solution (key, lock) {
    if (key.length < lock.length) {
        ...
        key = [...key.map(row => [...row, ...pad(gap)]), ...pad(gap, pad(lock.length))]
    }
    const m = key.length
    for (let rN = 0; rN < 4; rN++) {
        if (rN > 0) key = key[0].map((_, index) => key.map(row => row[index]).reverse())
        for (let vN = -m + 1; vN <= m - 1; vN++) {
            const vKey = key.map(row => vN >= 0
            ...
            for (let hN = -m + 1; hN <= m - 1; hN++) {
                ...
                if (lock.every((row, i) => row.every((n, j) => (n + hKey[i][j]) === 1))) return true
            }
        }
    }
    return false
